...
public class Main {
    ...
    public Main(){
        fields = new Field[]{
                ...
                new Field("ST depression induced by test", n-> n==1 ? "yes" : "no"),
                ...
                new Field("Result", n-> n==0 ? "low" : n==1 ? "moderate" : n==2 ? "high" : "very high")
        };
    }
    ...
    private void analyzeData() {
        ...
        for(int i : new int[]{0,1})
            System.out.println(printResult(fields[9].getValue(i), percentageAnalysis(n -> n[9]==i)));
    ...
    private double[] percentageAnalysis(Predicate<float[]> filter){
        long[] cntRes = new long[]{
                data.stream().filter(filter).filter(n -> n[13]==0).count(),
                data.stream().filter(filter).filter(n -> n[13]==2).count(),
                data.stream().filter(filter).filter(n -> n[13]==3).count(),
                data.stream().filter(filter).filter(n -> n[13]==4).count()
        };
        double sum = 0.0;
        for(long l : cntRes)
            sum+=l;
        if(sum==0.0)
            sum=1.0;
        return new double[]{cntRes[0]/sum, cntRes[1]/sum, cntRes[2]/sum, cntRes[3]/sum};
    }
